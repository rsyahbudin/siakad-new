...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Laporan Penugasan Guru</h1>
                <p class="text-gray-600 mt-1">Daftar lengkap penugasan guru untuk mengajar di setiap kelas</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>{{ $assignments->count() }} Penugasan</span>
                </div>
                <div class="flex items-center gap-2 text-sm text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                    </svg>
                    <span>{{ $assignments->unique('teacher_id')->count() }} Guru</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-gray-900">{{ $assignments->unique('teacher_id')->count() }}</div>
                    <div class="text-sm text-gray-600">Guru Terlibat</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-gray-900">{{ $assignments->unique('subject_id')->count() }}</div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-gray-900">{{ $assignments->unique('classroom_id')->count() }}</div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select id="dayFilter" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
                ...
                <select id="subjectFilter" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="">Semua Mata Pelajaran</option>
                    @foreach($assignments->unique('subject_id') as $assignment)
                    @if($assignment->subject)
                    <option value="{{ $assignment->subject->name }}">{{ $assignment->subject->name }}</option>
                    @endif
                    @endforeach
                </select>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Daftar Penugasan Guru</h2>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors assignment-row"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $a->teacher->full_name ?? '-' }}</div>
                                    <div class="text-xs text-gray-500">{{ $a->teacher->subject->name ?? 'Guru Umum' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                $dayColor = $dayColors[$a->day] ?? 'bg-gray-100 text-gray-800';
                                @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $dayColor }}">
                                    {{ $a->day }}
                                </span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-900">
                                    {{ \Carbon\Carbon::parse($a->time_start)->format('H:i') }} - {{ \Carbon\Carbon::parse($a->time_end)->format('H:i') }}
                                </span>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <div>
                                    <h3 class="text-lg font-medium text-gray-900 mb-2">Belum ada penugasan guru</h3>
                                    <p class="text-gray-600">Data penugasan guru akan muncul di sini setelah jadwal dibuat</p>
                                </div>
                            </div>
                        </td>
                    </tr>
