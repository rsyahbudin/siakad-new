    ...
    <form method="POST" action="{{ isset($jadwal) ? route('jadwal.admin.update', $jadwal) : route('jadwal.admin.store') }}">
        ...
        <div class="mb-4">
            ...
            <select name="day" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400 @error('day') border-red-500 @enderror">
                ...
                <option value="{{ $hari }}" {{ old('day', $day ?? $jadwal->day ?? request('day')) == $hari ? 'selected' : '' }}>{{ $hari }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="jam" min="1" max="10" value="{{ old('jam', $jam ?? request('jam')) }}" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400">
